...
var KeyboardedInput = function (_React$Component) {
  ...
  _createClass(KeyboardedInput, [{
    ...
    value: function componentWillUnmount() {
      ...
      if (this.state.focusLostTimer) clearTimeout(this.state.focusLostTimer);
    }
  }, {
    key: 'componentDidUpdate',
    value: function componentDidUpdate(prevProps, prevState) {
      if (this.state.showKeyboard !== prevState.showKeyboard && this.props.onToggle) this.props.onToggle(this.state.showKeyboard);
    }
  }, {
    key: 'handleChange',
      ...
      this.setState({
        focusLostTimer: setTimeout(function () {
          if (!document.activeElement.classList.contains("keyboard-button") && !document.activeElement.classList.contains("keyboard") && !document.activeElement.classList.contains("keyboard-row")) {
            _this2.setState(_extends({}, _this2.state, { showKeyboard: false, focusLostTimer: null }));
          }
        }, 0)
      });
    }
